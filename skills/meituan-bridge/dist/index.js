...
export default defineToolPlugin({
    id: "meituan-bridge",
    name: "美团AI 时空沙盒技能包",
    description: "美团本地生活全天候数字管家 — 14个核心技能通过 HTTP Bridge 注册为 OpenClaw Agent Tool（搜索/排程/防踩坑/异常/提醒/时钟/偏好/路径规划/排队监控/天气）",
    configSchema: Type.Object({
        backendUrl: Type.Optional(Type.String({ description: "Python Flask 后端地址，默认 http://localhost:5000" })),
    }),
    tools: (tool) => [
        // ── 1. POI搜索 + 全链路排程 ──
        tool({
            ...
            label: "POI搜索排程",
            description: "根据用户需求搜索美团本地生活商户，自动选店、填时间、生成并发排程任务时间线。适合「帮我找附近的川菜馆」「安排下午的理发+看电影」等综合需求。",
        ...
        tool({
            ...
            label: "读取管家偏好",
            description: "读取用户长期偏好：口味（辣度/忌口/菜系）、通勤（步行距离/交通方式）、预算（价格档位/评分门槛）、健康作息（喝水间隔/吃药时间）。",
            parameters: Type.Object({}),
            async execute(_params, config) {
                const base = config.backendUrl ?? DEFAULT_BACKEND;
        ...
        tool({
            ...
            label: "更新管家偏好",
            description: "更新用户偏好中的指定维度，支持口味、通勤、预算、健康作息四个维度的部分更新。",
            parameters: Type.Object({
                updates: Type.Record(Type.String(), Type.Unknown(), {
                    description: '要更新的偏好键值对，例如 {"taste": {"taste_tolerance": "中辣"}} 或 {"budget": {"price_level": "高端"}}',
                }),
            }),
        ...
        tool({
            ...
            label: "注入异常事件",
            description: "动态注入异常事件（店铺停电/排队满/天气预警/交通管制），触发异常传感器的 Plan B 容灾管线。",
            parameters: Type.Object({
                event_class: Type.String({
                    description: "事件类型：STORE_CLOSURE | QUEUE_FULL | WEATHER_EVENT | TRAFFIC_CONTROL",
                }),
                shop_name: Type.Optional(Type.String({ description: "受影响的店铺名称（不填则随机选一家）" })),
            }),
        ...
        tool({
            ...
            label: "防踩坑检查",
            description: "对当前排程管线做防踩坑检查，包括步行距离评估、交通模式建议、身份一致性校验（如宠物店→洗猫时确认是猫还是狗）。",
            parameters: Type.Object({
                pipeline_nodes: Type.Array(Type.Record(Type.String(), Type.Unknown()), {
                    description: "排程节点列表，每个节点包含 shop_name/transport/location 等字段",
                }),
                walking_tolerance_meters: Type.Optional(Type.Number({
        ...
        tool({
            ...
            label: "路径规划",
            description: "多节点路径规划：给定起点+途经点+交通偏好+天气，计算最优访问顺序与接驳方式（步行/打车/地铁）。",
            parameters: Type.Object({
                start_coord: Type.Optional(Type.String({ description: "出发坐标 lat,lng，默认三里屯" })),
                ...
                transport_preference: Type.Optional(Type.String({ description: "步行优先/打车优先/地铁优先" })),
                walking_tolerance_meters: Type.Optional(Type.Number({ description: "步行容忍距离（米）" })),
                weather_condition: Type.Optional(Type.String({ description: "天气状况，影响步行容忍距离" })),
            }),
        ...
        tool({
            ...
            label: "排队监控",
            description: "餐厅排队监控：排号入队、查询排号状态、批量轮询所有排队（≤5桌时自动提醒叫车）。",
            parameters: Type.Object({
        ...
        tool({
            ...
            label: "天气查询",
            description: "查询指定坐标+日期的天气状况，返回天气数据、活动建议（户外/室内）、交通影响（步行惩罚系数）。",
            parameters: Type.Object({
                coord: Type.Optional(Type.String({ description: "坐标 lat,lng，默认三里屯" })),
